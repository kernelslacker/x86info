/*
 *  $Id: identify.c,v 1.12 2001/08/10 10:47:52 davej Exp $
...
void identify (int cpunum)
{
	...
	switch (ebx) {
		...
		case 0x65736952:/* Rise */	Identify_Rise (cpunum, maxi, maxei, &cpu);	break;
	...
	if (!silent) {
		...
		switch (cpu.vendor) {
			...
			case VENDOR_CENTAUR:
				printf ("Family: %d Model: %d Stepping: %d [%s]\n",
					cpu.family, cpu.model, cpu.stepping, cpu.name);
				display_IDT_info (cpunum, maxei, &cpu);
				break;
			...
			case VENDOR_RISE:
				printf ("Family: %d Model: %d Stepping: %d [%s]\n",
					cpu.family, cpu.model, cpu.stepping, cpu.name);
				display_Rise_info (cpunum, maxi, maxei, &cpu);
				break;
